"""The version to study rho, velocity, pressure and relaxate the third equation"""
import torch
...
class EulerNet(torch.nn.Module):
    ...
    def interior_loss(self, x, weights=[1.0, 1.0, 1.0, 1.0]):
        ...
        second = rho * velocity**2 + pressure
        flux_true = velocity * (energy + pressure)
        dg_rho = gradients(rho, x)[0]
        dg_momentum = gradients(momentum, x)[0]
        dg_energy = gradients(energy, x)[0]
        dg_flux = gradients(flux, x)[0]
        dg_second = gradients(second, x)[0]
        rho_dt = dg_rho[:, 0:1]
        ...
        eq2 = self.loss_fn(momentum_dt + second_dx, zeros)
        eq3 = self.loss_fn(energy_dt + flux_dx, zeros)
        eqflux = self.loss_fn(flux - flux_true, zeros)
        L_relaxate = weights[0] * eq1 + weights[1] * eq2 + weights[2] * eq3
        L_flux = weights[3] * eqflux
        return L_relaxate, L_flux
