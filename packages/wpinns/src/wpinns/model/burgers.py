...
class BurgersNet(torch.nn.Module):
    def __init__(self, config: ConfigDict):
        ...
        test_configuration = getattr(
            config, "test_configuration", solution_configuration
        )
        ...
        if solution_layers is None:
            raise ValueError(
                "NetConfig must provide solution_layer_sizes or layer_sizes"
            )
        test_layers = getattr(
    ...
    def _cutoff_weight(self, x):
        ...
        if cutoff_name in {"quad", "quadratic"}:
            weight = (
                (1.0 - normalized.square()).clamp_min(0.0).prod(dim=-1, keepdim=True)
            )
            return weight / weight.amax().clamp_min(self.eps)
    ...
    def _norm(self, phi, phi_x):
        if self.entropy_norm == "H1":
            return torch.mean(
                torch.abs(phi) ** self.p + torch.abs(phi_x) ** self.p
            ) ** (1.0 / self.p)
    ...
    def _entropy_c_values(self, device, dtype):
        probe = torch.linspace(-1.0, 1.0, 2049, device=device, dtype=dtype).reshape(
            -1, 1
        )
        ...
        for c in c_values:
            ...
            if self.weak_form == "partial":
                residual = torch.mean(
                    ...
                    * (
                        grad_u_t * phi
                        - grad_phi_x * (0.5 * u.square() - 0.5 * c_scalar.square())
                    )
                )
                if self.use_relu:
