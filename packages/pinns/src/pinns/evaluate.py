...
from pinns.model import burgers, burgers_viscid, euler, swe
...
def _plot_state_slice(
    mode: str, root_dir: Path, slice_data: dict, save_path: Path | None
):
    actual_t = slice_data["actual_t"]
    epoch = slice_data["epoch"]
    if save_path is None:
        save_path = root_dir / "comparison_t_{}.pdf".format(
            _format_time_token(actual_t)
        )
...
def parse_args():
    ...
    parser.add_argument(
        "--path",
        required=True,
        help="Run directory containing config/history/checkpoints.",
    )
    parser.add_argument(
        "--mode",
        default="auto",
    ...
    parser.add_argument(
        "--time", type=float, default=0.6, help="Target time slice to plot."
    )
