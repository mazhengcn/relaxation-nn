...
def _build_optimization_schedule(model: torch.nn.Module, config: ConfigDict):
    ...
    if config.optimizer == "Adam_LBFGS":
        ...
        adam_optimizer, adam_scheduler = _build_adam_optimizer(
            ...
            decay=getattr(config, "adam_decay", getattr(config, "decay", "Exponential")),
            decay_rate=getattr(
...
def _default_cosine_t_max(total_epochs: int, scheduler_every: int):
    return max(1, (int(total_epochs) + int(scheduler_every) - 1) // int(scheduler_every))
...
def _record_state(
    ...
    if epoch % log_every == 0 or is_final:
        loss_log = " | ".join(
            "{} : {}".format(name, _to_float(loss_terms[name])) for name in history_terms
        )
        logging.info(
